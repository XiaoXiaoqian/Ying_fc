...
do
 ...
 for run in 1 2 3 4 5
    ...
    #   cp -R run${run}.feat/reg_standard run${run}_singletrial.feat
       applywarp --ref=run${run}.feat/reg/standard.nii.gz --in=run${run}_singletrial.feat/stats/pe1ls_one_at_time.nii.gz --out=run${run}_singletrial.feat/stats/pe1ls_one_at_time_2standard.nii.gz --warp=run${run}.feat/reg/highres2standard_warp.nii.gz --premat=run${run}.feat/reg/example_func2highres.mat --interp=trilinear
   done

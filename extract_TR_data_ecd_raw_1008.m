function extract_TR_data_ecd_raw_1008(sub,run)
% ecding TR changed to the 3th~4th TR. since the ss1 was only presented in
...
addpath /seastor/caiying/toolbox/NIFTI
   ...
   all_data_final_maint=zeros(size(all_data.img,1),size(all_data.img,2),size(all_data.img,3),12);
    ...
    filename=sprintf('%s/sub%02d/analysis/run%d_singletrial.feat/Extracted_ecd_TR_2standard_raw_1008.nii',basedir,sub,run);

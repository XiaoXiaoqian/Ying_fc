function extract_TR_data_ecd_raw_567(sub,run)
...
addpath /seastor/caiying/toolbox/NIFTI
   ...
   all_data_final_maint=zeros(size(all_data.img,1),size(all_data.img,2),size(all_data.img,3),12);
    ...
    for vol = 1:12
        if onset_ecd(vol)~=0
            tmp=(all_data.img(:,:,:,onset_ecd(vol))+(all_data.img(:,:,:,onset_maint(vol)+1))+(all_data.img(:,:,:,onset_maint(vol)+2)))/3;
    ...
    filename=sprintf('%s/sub%02d/analysis/run%d_singletrial.feat/Extracted_ecd_TR_2standard_raw_567.nii',basedir,sub,run);

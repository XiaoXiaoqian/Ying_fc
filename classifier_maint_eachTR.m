    ...
    for s=1:length(subs)
        ...
        for roi =1:length(roi_names)
            for roi=1:length(roi_names);
                ...
                data_all=[];xx=[];
                 % read the nueral activation in TR4 to train the classifier
                data_all=load(sprintf('%s/sub%02d/analysis/ROI_based_sub/%s/%s_item_TR04.txt',basedir,subs(s),roi_name,roi_name));
                x_all=data_all(4:end,:);% remove the final zero and the first three rows showing the coordinate
                x_all=sparse(x_all);
          
            % prepare the testing data,TRs on     
             for trs=1:14

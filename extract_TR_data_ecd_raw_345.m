function extract_TR_data_ecd_raw_345(sub,run)
...
addpath /seastor/caiying/toolbox/NIFTI
   ...
   all_data_final_maint=zeros(size(all_data.img,1),size(all_data.img,2),size(all_data.img,3),12);
    ...
    filename=sprintf('%s/sub%02d/analysis/run%d_singletrial.feat/Extracted_ecd_TR_2standard_raw_345.nii',basedir,sub,run);

function extract_TR_data_maint_raw_678(sub,run)
...
%localizer
   ...
   all_data_final_maint=zeros(size(all_data.img,1),size(all_data.img,2),size(all_data.img,3),12);
    ...
    for vol = 1:12
        if onset_ecd(vol)~=0
            tmp=(all_data.img(:,:,:,onset_maint(vol)-1)+all_data.img(:,:,:,(onset_maint(vol)))+all_data.img(:,:,:,(onset_maint(vol)+1)))/3;
            % demean in each run
    ...
    filename=sprintf('%s/sub%02d/analysis/run%d_singletrial.feat/Extracted_maint_TR_2standard_raw_678.nii',basedir,sub,run);
